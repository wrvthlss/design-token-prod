...
const isValidColor = (value) => {
  ...
  const rgbaPattern =
    /^rgba?\(\s*(\d{1,3}\s*,\s*){2,3}\d{1,3}\s*(,\s*\d(\.\d+)?\s*)?\)$/;
...
StyleDictionary.registerTransform({
  ...
  transformer: (token) => {
    if (!isValidColor(token.value)) {
      console.error(
        `Error: Token '${token.name}' has an invalid color value: '${token.value}'.`,
      );
...
StyleDictionary.registerTransform({
  ...
  transformer: (token) => {
    if (!isValidDimension(token.value)) {
      console.error(
        `Error: Token '${token.name}' has an invalid dimension value: '${token.value}'. Dimensions should have units (e.g., px, rem).`,
      );
...
StyleDictionary.registerTransform({
  ...
  type: 'value',
  matcher: (token) =>
    token.attributes && token.attributes.category === 'fontSize',
...
StyleDictionary.registerTransform({
  ...
  type: 'value',
  matcher: (token) =>
    token.attributes && token.attributes.category === 'spacing',
