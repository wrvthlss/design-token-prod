...
import { ofErrorKeywords, parentKeywordsOk, areErrorsEqual, getCommonPrefix, getDataReferencePath, getOfChoiceIndex, getPropertyMatchScore, getSchemaType, getType, getValueMatchScore, } from './util.js';
/**
 ...
 */
export function getSimpleErrorText(errors, options = {}) {
...
export function getSimpleErrors(errors, options = {}) {
    ...
    const simpleErrors = processedErrors.map((error) => {
        ...
        const { instancePath, schemaPath, schema, parentSchema, data, } = error;
        return {
            message,
            instancePath,
            schemaPath,
...
function getOfHierarchy(errors) {
    const ofs = errors.filter((err) => isOfError(err));
    if (ofs.length === 0) {
        return errors;
    }
    ...
    for (const error of nonOfErrors) {
        ...
        for (const schemaPath of schemaOfs) {
            // already stored
            const storedErrors = ref.flat();
            // Fixed https://github.com/ghmcadams/vscode-lintlens/pull/78
                ...
                errors.forEach(otherError => {
                    if (otherError.instancePath === error.instancePath) {
                        ...
                        if (otherError.schemaPath.slice(schemaPathReplacement.length + 1).includes('/')) {
                            schema[index] = otherError.parentSchema;
                        }
                    }
                });
            }
...
function getChosenPaths(error) {
    const { choices, data, schema } = error;
